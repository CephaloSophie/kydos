...
export class TableService {
  ...
  /**
   * Liste paginée des tables en LOBBY et des parties EN COURS (SPEC §3.3).
   * Portées : `all` (publiques + les miennes + mon équipe), `team`, `public`.
   * Les parties en cours restent visibles (statut « en cours ») pour permettre
   * de les REGARDER. Pagination : 15 par page.
   */
  async listOpenTables(userId: string, options: { scope?: string; page?: number } = {}) {
    const PAGE_SIZE = 15;
    ...
    if (scope === 'public') visibilityFilter = { visibility: 'public' };
    else if (scope === 'team') visibilityFilter = userDocument?.team ? { ownerType: 'team', team: userDocument.team } : { _id: null };
    else {
    ...
    // Lobby OU en cours (on garde les parties lancées visibles pour les regarder).
    const filter = { $and: [visibilityFilter, { status: { $in: ['lobby', 'playing'] } }] };
