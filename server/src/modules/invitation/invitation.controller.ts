...
export class InvitationController {
  ...
  async cancel(request: AuthenticatedRequest, response: Response) { await invitationService.cancel(request.params.id, request.userId!); response.json({ ok: true }); }
  async countReceived(request: AuthenticatedRequest, response: Response) { response.json({ count: await invitationService.countReceived(request.userId!) }); }
}
