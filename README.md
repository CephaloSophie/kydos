...
Monorepo : **moteur partagé** (TypeScript) + **couche application** + **backend Node/MongoDB modulaire** + **frontend React**.
...
│                         scoring (carte + récompense), cerveau des robots, GameEngine,
│                         et la FABRIQUE de robot `robotFromFiche` (voir plus bas).
...
├─ web/                   React + Vite. La table de jeu est un MODULE réutilisable (web/src/table).
