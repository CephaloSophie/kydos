  ...
  return (
    <div className="px-hud">
      ...
      <div className="px-toolbar">
        {props.showScoreSheet !== false && <TornScore history={cumulHistory.current} manchesA={view.manchesWon.A} manchesB={view.manchesWon.B} onClick={() => setShowScore(true)} />}
      ...
      {mySeat != null && (
        <div className="px-actions-slot">
          ...
          {props.onEmote && (
            <div className="ky-emotes">
              {['😂', '😊', '😢', '🥂'].map((e2) => (
                <button key={e2} className="ky-emotes__btn" disabled={view.phase === 'bidding' || view.phase === 'surcontre'}
                  onClick={() => props.onEmote!(e2)}>{e2}</button>
              ))}
            </div>
          )}
        </div>
      ...
      {showScore && <ScoreSheet view={view} summary={summary} onClose={() => setShowScore(false)} />}
    </div>
  );
}
