/* =============================================================================
 * TABLE-PIXI · robotMascot.ts — Mascotte robot PARAMÉTRIQUE (SVG) + rasterisation.
 * -----------------------------------------------------------------------------
 * v18 — Générateur SVG partagé (mobile + rendu Pixi) : un petit robot amical
 * dessiné par code à partir de couleurs (accent = yeux/antenne, corps, contour).
...
/** SVG de la mascotte. corps/contour dérivés de l'accent si absents. */
...
export function faceKey(face: RobotFace): string {
  return `${face.accentColor}|${face.bodyColor ?? ''}|${face.outlineColor ?? ''}`;
}
 ...
 */
export function mascotTexture(face: RobotFace, px = 96, onReady?: () => void): Texture | null {
  const key = `${faceKey(face)}@${px}`;
  const cached = texCache.get(key);
  if (cached) return cached;
  try {
    const svg = robotMascotSvg(face, px);
    const img = new Image();
    img.onload = () => {
      try {
