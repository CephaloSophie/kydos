...
import { handInset, type Dir, type Rect } from './geometry';
...
export class HandsLayer extends Container {
  ...
  update(
    ...
    for (let rel = 0; rel < 4; rel++) {
      ...
      // Règle d'affichage constante et lisible :
      //   • SUD (rel 0) = la main du joueur → toujours face visible et jouable.
      //   • Les 3 autres sièges = toujours DOS (jamais les cartes des autres),
      //     même en mode démonstration. `mode === 'faceup'` reste possible en
      //     débogage web mais NE révèle jamais le coéquipier.
      const interactive = rel === 0;
      const faceUp = interactive || (mode === 'faceup' && !(rel === 2 && partnerFaceDown));
      const shown = faceUp && cards ? displaySort(cards, view.trump) : cards!;
      // Les cartes des 3 AUTRES joueurs sont réduites de 30% (0.7×) par rapport
      // à la main du joueur courant (SUD), pour hiérarchiser la lecture.
      const seatCardW = interactive ? cardW : Math.round(cardW * 0.7);
      this.drawFan(dir, shown, count, faceUp, interactive, legal, view, mySeat, rect, atlas, seatCardW, onPlay);
    }
  }
  ...
  private drawFan(
    ...
    // Ancrage proportionnel (fonction partagée avec TrickLayer).
