...
import { handInset, type Rect } from './geometry';
import { placeTrickCard, type TrickPlacement } from './trickPlacement';
...
export class TrickLayer extends Container {
  ...
  private collectKey = '';
  // Placement figé par carte (offset + rotation), calculé une fois puis réutilisé
  // pour rester stable entre deux rendus. Effacé à chaque collecte.
  private placement = new Map<string, TrickPlacement>();
  private playOrder = 0; // ordre d'empilement (la dernière carte jouée passe au-dessus)
  constructor(private theme: PixiTableTheme) { super(); this.sortableChildren = true; }
  setTheme(t: PixiTableTheme) { this.theme = t; }

  /**
   * Cible d'une carte jouée : proche du CENTRE de la table, avec un décalage
   * aléatoire borné (≤ 40% de la taille de carte) et une inclinaison
   * perpendiculaire au joueur ±30°. Le calcul est délégué au helper pur
   * `placeTrickCard` (testé) ; on mémorise le résultat pour le figer.
   */
  private target(key: string, rel: number, rect: Rect, cardW: number, cardH: number) {
    if (!this.placement.has(key)) this.placement.set(key, placeTrickCard(key, rel, cardW, cardH));
    const p = this.placement.get(key)!;
    const cx = rect.x + rect.w / 2, cy = rect.y + rect.h / 2;
    return { x: cx + p.dx, y: cy + p.dy, rot: p.rot };
  }

  /** Where a card COMES FROM: the hand anchor of its seat (shared inset). */
  private handOrigin(dir: DirName, rect: Rect, cardH: number) {
    const INSET = handInset(cardH);
    switch (dir) {
  ...
  update(view: EngineView, mySeat: Seat, rect: Rect, atlas: CardAtlas, cardW: number) {
    ...
    //    Chaque nouvelle carte passe AU-DESSUS des précédentes (zIndex croissant).
  ...
  /**
   * Glissé depuis la main : 380 ms avec une courbe douce (easeOutCubic), un
   * léger arc vertical et une petite montée d'échelle en fin de course, pour
   * que le mouvement paraisse posé plutôt que téléporté.
   */
  private animatePlay(cv: CardSprite, from: { x: number; y: number }, to: { x: number; y: number; rot: number }) {
    const startRot = to.rot + 0.14;
    cv.rotation = startRot;
    cv.alpha = 0.6;
    let elapsed = 0;
    const DUR = 380;
    const arc = Math.min(28, Math.hypot(to.x - from.x, to.y - from.y) * 0.12);
    const tick = (t: Ticker) => {
      if (cv.destroyed) { Ticker.shared.remove(tick); return; }
      elapsed += t.deltaMS;
      const p = Math.min(1, elapsed / DUR);
      const e = easeOut(p);
      const lift = Math.sin(p * Math.PI) * arc; // arc qui s'annule en début/fin
      cv.position.set(from.x + (to.x - from.x) * e, from.y + (to.y - from.y) * e - lift);
      cv.rotation = startRot + (to.rot - startRot) * e;
      cv.alpha = 0.6 + 0.4 * e;
      const sc = 1 + 0.06 * Math.sin(p * Math.PI);
      cv.scale.set(sc, sc);
      if (p >= 1) { cv.scale.set(1, 1); cv.alpha = 1; Ticker.shared.remove(tick); }
    };
