...
- **Mobile** : `mobile/dist/` embarqué par Capacitor (voir ci-dessous).
...
cd mobile
npx cap add android                      # 1re fois → mobile/android/
npx cap sync                             # copie dist → natif
npm --workspace belote-mobile run cap:android   # ou ouvrir dans Android Studio pour un APK release
```
