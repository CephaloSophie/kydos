...
## Historique détaillé, stats et parties robots (v11.3.0)

### Fin de partie — libération du verrou
À la persistance d'une partie, `singleGameLockService.releaseAllOf(tableId)`
libère le verrou de partie unique de TOUS les joueurs (le verrou stocke l'ID de
table). Sans ça, la bannière « Partie en cours » resterait et les robots
seraient vus comme occupés.

### GET /api/games — paginé et filtré
Query : `scope` (`mine` | `myrobots` | `team` | `public`), `page`, `kind`.
Réponse : `{ games[], page, pageSize (15), total, totalPages }`. Chaque partie
inclut `finalScore`, `manchesWon`, `stats` (plis, contrées, capots...) et la
composition (`participants`).

### GET /api/tables — paginé, lobby + en cours
Query : `scope` (`all` | `team` | `public`), `page`. Réponse paginée (15). Inclut
désormais les tables en `lobby` ET les parties `playing` (visibles pour être
regardées).

### POST /api/games/robots — match 100% robots
Body : `{ robotIds[4], visibility?, forTeam?, manches? }`. Simule la partie d'un
trait côté serveur (aucune socket, pas de session live, absente de la liste des
parties en cours) et l'archive dans l'historique. Réponse :
`{ gameId, winner, scoreA, scoreB }`.

### Statistiques (belote-core)
`computeGameStats(replay)` dérive du replay : plis par équipe, contrées et
surcontrées et leur réussite, capots réalisés et annoncés, belotes, score final.
Ces valeurs sont embarquées dans le document Game à la persistance.

---

## Invitations d'équipe (v11.5.0)

Module `invitation` — flux complet :

- `POST /api/teams/:id/invite` — le propriétaire invite (body `{ identifier }` :
  pseudo, e-mail ou id).
- `GET /api/invitations` — invitations REÇUES en attente.
- `GET /api/invitations/count` — nombre reçu (badge de notification).
- `GET /api/teams/:id/invitations` — invitations ENVOYÉES en attente (propriétaire).
- `POST /api/invitations/:id/accept` — accepter (rejoint l'équipe ; respecte la
  borne de 40 membres, ajoute au tableau `members`, refuse si déjà dans une
  autre équipe).
- `POST /api/invitations/:id/decline` — refuser (destinataire).
- `POST /api/invitations/:id/cancel` — annuler (expéditeur/propriétaire).
- `GET /api/users/search?q=` — recherche de joueurs par pseudo (pour inviter).

Statuts d'invitation : `pending` | `accepted` | `declined` | `cancelled`.

---

## Profil joueur & retour de partie (v11.6.0)

- `GET /api/users/:id/profile` — enrichi : `{ id, username, level, rank,
  rewardPoints, gamesPlayed, wins, losses, robotCount, robots[], team }`. Les
  robots : `{ id, name, elo, avatarId }`. Bilan wins/losses calculé depuis les
  parties archivées où le joueur a participé.
- Broadcast `table:game` — `view.players` ajouté : `[{ seat, name, type,
  userId }]` (pour ouvrir un profil depuis la table).
- `GET /api/games` & détail — chaque participant expose désormais `userId`.
