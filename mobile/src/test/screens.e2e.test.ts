...
describe('Reprise de partie — la bannière mène au mode en ligne (pas de dialogue)', () => {
  it('affiche la bannière quand une partie est en cours', async () => {
    uninstall();
    uninstall = installFakeServer({ activeSession: 'tbl_active' });
    const el = await mount(HomeScreen);
    expect(el.textContent).toContain('Partie en cours');
    expect(el.textContent).toContain('Reprendre la partie');
  });

  it('n\'affiche PAS la bannière sans partie en cours', async () => {
    uninstall();
    uninstall = installFakeServer();
    const el = await mount(HomeScreen);
    // La bannière reste masquée (display:none) tant qu'il n'y a pas d'activeSession.
    const banner = Array.from(el.querySelectorAll('div')).find((d) => d.textContent?.includes('Partie en cours…'));
    expect(banner == null || (banner as HTMLElement).style.display === 'none').toBe(true);
  });
