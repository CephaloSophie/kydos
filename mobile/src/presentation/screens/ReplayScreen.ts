...
const delayOf = (op: Operation): number => (op.type === 'play' ? 900 : op.type === 'bid' ? 700 : op.type === 'trick' ? 1200 : 400);
...
export function ReplayScreen(ctx: AppContext): HTMLElement {
  ...
  const speedChip = chip('Vitesse 1×', { background: 'rgba(16,21,31,.85)', border: '1px solid var(--c-line)', color: 'var(--c-text-soft)' }, () => {
    speed = speed === 1 ? 2 : speed === 2 ? 4 : 1; speedChip.textContent = `Vitesse ${speed}×`;
  });
  ...
  const applyOp = ({ donne, op }: { donne: number; op: Operation }) => {
    ...
    const data = op.data as { seat?: Seat; bid?: Bid; card?: Card; A?: number; B?: number };
    if (op.type === 'bid' && data.bid && data.seat != null) state!.bids.push({ ...data.bid, seat: data.seat });
    if (op.type === 'play' && data.card && data.seat != null) {
      state!.trick.push({ seat: data.seat, card: data.card });
      state!.hands[data.seat] = state!.hands[data.seat].filter((x) => !(x.rank === data.card!.rank && x.suit === data.card!.suit));
    }
  ...
  const tick = () => {
    ...
    timer = setTimeout(() => { timer = null; applyOp(item); render(); tick(); }, Math.max(60, delayOf(item.op) / speed));
  };
